...
export class SensorFormatter {
  private logger: Logger;

  constructor() {
    this.logger = new Logger('SensorFormatter');
  }
   ...
   */
  private getPingIcon(state: string): string {
    switch (state) {
      case 'online':
   ...
   */
  private getDNSIcon(state: string): string {
    switch (state) {
      case 'available':
        return 'mdi:dns';
      case 'slow':
   ...
   */
  private isPingSensorAvailable(sensorState: PingSensorState): boolean {
   ...
   */
  private isDNSSensorAvailable(sensorState: DNSSensorState): boolean {
   ...
   */
  private getPingPerformanceStatus(result: PingResult, sensorState: PingSensorState): string {
    if (!result.success) {
      return 'failed';
    }
   ...
   */
  private getDNSPerformanceStatus(result: DNSResult, sensorState: DNSSensorState): string {
    if (!result.success) {
      return 'failed';
    }
   ...
   */
  private isValidDeviceClass(deviceClass: string): boolean {
   ...
   */
  createComprehensiveAttributes(
    baseAttributes: SensorAttributes,
