...
import { NetworkError, SystemHealth, ErrorCategory, ErrorSeverity } from './error-types';
...
export class ErrorReporter {
   ...
   */
  async reportError(error: NetworkError, context?: Record<string, any>): Promise<void> {
   ...
   */
  async reportRecoveryAttempt(attempt: RecoveryAttempt, error?: NetworkError): Promise<void> {
    const report: ErrorReport = {
      id: this.generateReportId(),
      timestamp: new Date(),
   ...
   */
  async reportSystemHealth(health: SystemHealth): Promise<void> {
    const report: ErrorReport = {
      id: this.generateReportId(),
      timestamp: new Date(),
      report_type: 'health',
  ...
  async exportReports(
    ...
    const filepath = join(this.config.log_directory, filename);
  ...
  getErrorStatistics(): {
    ...
    error_trends: { date: string; count: number }[];
  } {
   ...
   */
  private formatErrorReport(error: NetworkError, context?: Record<string, any>): string {
   ...
   */
  private formatRecoveryReport(attempt: RecoveryAttempt, error?: NetworkError): string {
   ...
   */
  private formatHealthReport(health: SystemHealth): string {
   ...
   */
  private async writeToLogFile(logType: string, report: ErrorReport): Promise<void> {
    try {
   ...
   */
  private logToConsole(error: NetworkError, report: ErrorReport): void {
  ...
  private async ensureLogDirectory(): Promise<void> {
    try {
      if (!existsSync(this.config.log_directory)) {
        await mkdir(this.config.log_directory, { recursive: true });
