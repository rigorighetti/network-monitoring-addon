...
class DataStore {
    constructor(logger, dataDir = '/data', retentionDays = 30) {
        this.db = null;
        this.logger = logger;
        this.dbPath = path_1.default.join(dataDir, 'network_monitoring.db');
     ...
     */
    async getPingHistory(targetName, timeRange, limit = 1000) {
        const sql = `
     ...
     */
    async getDnsHistory(serverName, timeRange, limit = 1000) {
        const sql = `
    ...
    async getStats() {
        ...
        return new Promise((resolve, reject) => {
            ...
            let pingCount = 0;
            let dnsCount = 0;
            this.db.get(pingCountSql, (err, row) => {
                if (err) {
                    reject(err);
                    return;
                }
                pingCount = row.count;
                this.db.get(dnsCountSql, (err, row) => {
                    if (err) {
                        reject(err);
                        return;
                    }
     ...
     */
    async getAggregatedPingData(targetName, timeRange, intervalMinutes = 5) {
     ...
     */
    async getAggregatedDnsData(serverName, timeRange, intervalMinutes = 5) {
        const intervalSeconds = intervalMinutes * 60;
        const sql = `
      SELECT 
        (timestamp / ${intervalSeconds}) * ${intervalSeconds} as time_bucket,
     ...
     */
    async getAggregatedDnsDataByType(serverName, queryType, timeRange, intervalMinutes = 5) {
        const intervalSeconds = intervalMinutes * 60;
        const sql = `
      SELECT 
        (timestamp / ${intervalSeconds}) * ${intervalSeconds} as time_bucket,
     ...
     */
    async getRecentPingData(targetName, minutes = 60) {
    ...
    `;
        return new Promise((resolve, reject) => {
            ...
            this.db.all(sql, [targetName, cutoffTimestamp], (err, rows) => {
                if (err) {
                    reject(err);
                    return;
                }
     ...
     */
    async getRecentDnsData(serverName, minutes = 60) {
        const cutoffTimestamp = Math.floor((Date.now() - (minutes * 60 * 1000)) / 1000);
        const sql = `
      SELECT timestamp, server_name, server_ip, domain, query_type,
             response_time_ms, success, resolved_address, error_message
    ...
    `;
        return new Promise((resolve, reject) => {
            ...
            this.db.all(sql, [serverName, cutoffTimestamp], (err, rows) => {
                if (err) {
                    reject(err);
                    return;
                }
     ...
     */
    async getPingSummary(targetName, timeRange) {
        const sql = `
      SELECT 
     ...
     */
    async getDnsSummary(serverName, timeRange) {
        const sql = `
      SELECT 
        COUNT(*) as total_tests,
        SUM(CASE WHEN success = 1 THEN 1 ELSE 0 END) as successful_tests,
    ...
    async getAvailableTargets() {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.db.all(pingTargetsSql, (err, pingRows) => {
                if (err) {
                    reject(err);
                    return;
                }
                this.db.all(dnsServersSql, (err, dnsRows) => {
                    if (err) {
                        reject(err);
                        return;
                    }
