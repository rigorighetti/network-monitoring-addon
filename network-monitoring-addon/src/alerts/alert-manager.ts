...
import { Alert, AlertRule, AlertCondition, NotificationConfig, AlertManagerState } from '../types/alerts';
import { PingResult, DNSResult, MonitoringStatus } from '../types/results';
...
export class AlertManager extends EventEmitter {
  ...
  private targetStates: Map<string, MonitoringStatus> = new Map();
  ...
  addAlertRule(rule: AlertRule): void {
    ...
    logger.debug(`Alert rule added/updated: ${rule.name}`);
  }
  ...
  addNotificationConfig(config: NotificationConfig): void {
    ...
    logger.debug(`Notification config added/updated: ${config.service}`);
  }
  ...
  private initializeDefaultRules(): void {
    ...
    const pingFailureRule: AlertRule = {
    ...
    const pingLatencyRule: AlertRule = {
    ...
    const pingPacketLossRule: AlertRule = {
      id: 'ping_packet_loss',
    ...
    // DNS failure rule
    const dnsFailureRule: AlertRule = {
    ...
    const dnsLatencyRule: AlertRule = {
      id: 'dns_high_latency',
   ...
   */
  private updateTargetState(targetKey: string, result: PingResult | DNSResult): void {
    ...
    const status: MonitoringStatus = {
      ...
      ...((result as PingResult).packet_loss_percent !== undefined && {
        current_packet_loss: (result as PingResult).packet_loss_percent
      })
    };
   ...
   */
  private determineStatus(result: PingResult | DNSResult): MonitoringStatus['status'] {
    if (!result.success) {
   ...
   */
  private checkAlertConditions(targetKey: string, targetType: 'ping' | 'dns', result: PingResult | DNSResult): void {
   ...
   */
  private evaluateAlertCondition(rule: AlertRule, result: PingResult | DNSResult, targetState: MonitoringStatus): boolean {
    ...
    switch (condition.metric) {
      case 'response_time':
        ...
        break;
      case 'packet_loss':
        metricValue = (result as PingResult).packet_loss_percent || 0;
        break;
      case 'availability':
   ...
   * Generate an alert
   */
  private generateAlert(rule: AlertRule, targetState: MonitoringStatus, result: PingResult | DNSResult): void {
   ...
   */
  private checkRecoveryCondition(targetKey: string, result: PingResult | DNSResult): void {
    const targetState = this.targetStates.get(targetKey);
    if (!targetState) {
      return;
    }
    ...
    if (targetAlerts.length > 0) {
      ...
      logger.info(`Recovery detected: ${recoveryAlert.message}`);
   ...
   */
  private determineAlertType(rule: AlertRule): Alert['type'] {
   ...
   */
  private determineSeverity(rule: AlertRule, targetState: MonitoringStatus): Alert['severity'] {
   ...
   */
  private generateAlertMessage(rule: AlertRule, targetState: MonitoringStatus, result: PingResult | DNSResult): string {
    ...
    switch (rule.condition.metric) {
      ...
      case 'packet_loss':
        const packetLoss = (result as PingResult).packet_loss_percent || 0;
