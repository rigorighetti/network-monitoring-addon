...
import { NetworkError, ErrorCategory, ComponentHealth } from './error-types';
...
export class RecoveryManager extends EventEmitter {
   ...
   */
  async attemptRecovery(error: NetworkError): Promise<boolean> {
  ...
  getRecoveryStatistics(): {
    ...
    const attemptsByComponent: Record<string, number> = {};
   ...
   */
  async forceRecovery(component: string, actionName: string): Promise<boolean> {
   ...
   */
  clearRecoveryHistory(maxAge: number = 7 * 24 * 60 * 60 * 1000): void {
  ...
  private setupErrorHandlerListeners(): void {
    this.errorHandler.on('error', async (error: NetworkError) => {
   ...
   */
  private async executeRecoveryAction(error: NetworkError, action: RecoveryAction): Promise<boolean> {
   ...
   */
  private isInCooldown(actionKey: string): boolean {
   ...
   */
  private setCooldown(actionKey: string, cooldownMs: number): void {
   ...
   */
  private getRecoveryAttempts(errorId: string): RecoveryAttempt[] {
   ...
   */
  private recordRecoveryAttempt(errorId: string, attempt: RecoveryAttempt): void {
   ...
   */
  private shouldAttemptAutoRecovery(error: NetworkError): boolean {
    ...
    if (error.category === ErrorCategory.PERMANENT_FAILURE) {
      return false;
    }
